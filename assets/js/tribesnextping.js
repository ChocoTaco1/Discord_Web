$(document).ready(function() {
	...
	function updateView(data) {
		var template =
			...
			data.num_players +
			'/' +
			data.info_flags.max_players +
			'</div>';
		template = '<div>' + template + data.info_map + '&nbsp; / &nbsp;' + data.info_maptype + '</div>';
		$serverStatContainer.html(
			'<div bgcolor="" style="font-size:16px;line-height: 17px;"><a href="server.html" style="text-align: center; color:#545c61;">' +
				template +
				'</a></div>'
		);
	}
	...
	function FULLupdateView(data) {
		...
		template =
			template +
			'<div>there\'s <strong style="color:#0a9ba8;">' +
			data.num_players +
			'</strong> player' +
			(data_players != 1 ? 's' : '') +
			' on</div>';
		...
		$FULLserverStatContainer.html(
			'<div bgcolor="" style="line-height: 100px;font-size:85px;text-align: center;">' + template + '</div>'
		);
	}
